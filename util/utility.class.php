<?php
	...
	// Comment out the variable locator in order to hardcode the envorinment file.
	// Variable selector
	define("CR_INI_PATH", dirname($_SERVER['DOCUMENT_ROOT']) . DIRECTORY_SEPARATOR . 'env.' . basename(dirname(dirname(__FILE__))) . '.ini');
	// Vimexx example:
	// define("CR_INI_PATH", '/home/u70472p67165/domains/trb.nu/env.ini');
	// Local development
	// define("CR_INI_PATH", 'D:/xampp/htdocs/local-env.ini');
	...
	function requiresPrivilege($requiredPrivilege) {
		
		//retrieve current CR privileges table
		$database = new Database();
		$database->query('SELECT * FROM `cr_privileges`');	
		$privilegesTable = $database->resultset();	
		
		//retrieve current user groups --> vervangen door een O365 versie van getUserName()?
		$user = new MSGraphUser($_SESSION['token']);
		$userGroups = $user->getGroups();
		$userGroupsJSON = json_encode($userGroups);
		$userGroupsValues = json_decode($userGroupsJSON);
		...
		//compair current user groups against CR privileges table Grants
		foreach ( $privilegesTable as $privilege => $privilegeGrant ) {
			foreach ( $userGroupsValues as $group ) {
				foreach ( $group as $groupAttribute => $groupAttributeValue ) {			
					if ( $privilegeGrant["object_id"] === $groupAttributeValue ) {
						if ( $privilegeGrant[$requiredPrivilege] == '1' ) {
							return true;
						}else {
							return 'This permission has not been granted to user '.$userName; //Was een functie call naar getUserName() wellicht deze updaten naar een MS variant.
						}
					}
				}
    ...
    function updatePasfotoTable($user)
    {
        try {
            ...
            foreach ($queryResult as $key => $value) {
                if (file_exists($crbinRoot.'/crbin/pasfotos/'.$value['Relatienr'].'.jpg')) {
    ...
    function opschonenPasfotos($user)
    {
        try {
            ...
            $dir = new DirectoryIterator($crbinRoot.'/crbin/pasfotos/');
            $count = 0;
            foreach ($dir as $fileinfo) {
                if (!$fileinfo->isDot() && !$fileinfo->isDir()) {
                    ...
                    if (in_array($relatienr, $haystack) === false && $relatienr !== 'pasfoto') {
                        $success = rename($fileinfo->getPathname(), $crbinRoot.'/crbin/pasfotos/delete/'.$relatienr.'.jpg');
                        if ($success) {
                            $count++;
                        };
                    };
    ...
    function clean_post_array($array)
    {
        $returnArray = array();

        foreach ($array as $key=>$val) {
            $returnArray[htmlspecialchars($key)] = htmlspecialchars($val);
        }
     
        return $returnArray;
    };

    function check_array_empty_values($array)
    {
        $stringEmpty = '';
    ...
    function getProfilePictureSource($relatienr)
    {
        ...
        if (file_exists($crbinRoot.'/crbin/pasfotos/'.$relatienr.'.jpg')) {
            //HTTP Auth voor de pasfotos zit in de url.
            $path = $crbinRoot.'/crbin/pasfotos/'.$relatienr.'.jpg';
        } else {
            $path = $_SERVER['DOCUMENT_ROOT'] . '/clubredders/images/crm-photo-notavailable.jpg';
        }
