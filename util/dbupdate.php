...
if ($base->input['submit'] == "Update")
{
	...
	if ($databaseversion == "2.3") {
	  ...
	  // add a field to indicate einvoice_type
	  $query = "ALTER TABLE `billing` ADD `einvoice_type` ".
	    "ENUM( 'txt', 'pdf' ) NOT NULL DEFAULT 'txt'";
	  $result = $DB->Execute($query) or die ("$query failed");
	  echo "$query<br>\n";
	  
	  $query = "ALTER TABLE `general` ADD `einvoice_footer` TEXT NULL ".
	    "AFTER `invoice_footer` ";
	  $result = $DB->Execute($query) or die ("$query failed");
	  echo "$query<br>\n";
	...
	if ($databaseversion == "2.4") {
	  // set the version number in the database to 2.4.1
	  $query = "UPDATE `settings` SET `version` = '2.4.1' ".
	    "WHERE `id` =1 LIMIT 1";
	  $result = $DB->Execute($query) or die ("$query failed");
	  echo "$query<br>\n";	  
	}
...
else 
{
	...
	This script will update it to version: <b>2.4.1</b></h3>";

	if ($databaseversion == "2.4.1") {
