...
// De eerste regel van de array krijgt een eigen prefix. Deze gaan we later matchen met de benodigde gegevens in de kaart.
	...
	foreach ( $tableResult as $competentie ) {
		array_push($numRows, $competentie[Row]);
		array_push($numColumns, $competentie[Column]);
	}
	
	array_unique($numRows);
	array_unique($numColumns);
	
	foreach ( $numColumns as $varColumn ) {
		${'Column'.$competentie[Column]} = (object) ["Column" => $competentie[Column], "Name" => "C$competentie[Column]"];
		array_push($Columns, ${'Column'.$competentie[Column]});
	}
	foreach ( $numRows as $varRow ) {	
		${'Row'.$competentie[Row]} = (object) ['Row' => $competentie[Row], 'Name' => "R$competentie[Row]", 'Columns' => $Columns];
		array_push($array, ${'Row'.$competentie[Row]});
	}
